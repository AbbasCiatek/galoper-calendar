...
import {formatDate, getHours, getMinutes, isDate, isThisYear, setHours, setMinutes} from "date-fns";
...
export default function DatePicker({id,value,onSelect} : DatePickerProps) {
    ...
    const defaultMinutes = (isDate(value)) ?   getMinutes(value) : getMinutes(new Date());
    const [time, setTime] = useState(`${String(defaultHours).padStart(2, "0")}:${String(defaultMinutes).padStart(2, "0")}`);
            ...
            <div className="flex flex-col gap-3">
                <Popover open={open} onOpenChange={setOpen}>
                    <PopoverTrigger asChild>
                        ...
                        >
                            {isDate(value) ? isThisYear(value) ? formatDate(value,"EEEE, MMMM d") : formatDate(value,"EEEE,  MMMM d, y ") : "Select date"}
                            <ChevronDownIcon />
                        </Button>
                    </PopoverTrigger>
