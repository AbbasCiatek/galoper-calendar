...
import {MultiDayEventDisplay} from "@/Components/WeekComponents/MultiDayEventDisplay.tsx";

export default function WeekViewMultiDayEvent({date}:{date:Date}){
    const {getMultipleDayEvents} =useEventStore();
    const multiDayEvents = getMultipleDayEvents(startOfWeek(date,{weekStartsOn:1}),endOfWeek(date,{weekStartsOn:1}));
                ...
                {weekDays.map((day, dayIndex) => (
                    <div key={day.toISOString()} className="flex h-full flex-col gap-1 py-1">
                        {eventRows.map((row, rowIndex) => {
                            ...
                            return <MultiDayEventDisplay key={`${event.id}-${dayIndex}`} event={event} cellDate={startOfDay(day)} position={position} />;
                        })}
                    </div>
                ))}
            </div>
